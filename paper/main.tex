...
\title{Disjunctive dual-stream defense of live telephone calls: operational fusion, mid-call production change, and a sufficient-statistic agent}
...
\begin{itemize}
\item On a lexicon-locked independent script set, a wide bag of the frozen stage lexicon beats narrow keywords; the HMM stage tracker does \emph{not} beat the wide bag. Trap means are reported alongside AUC.
\item Operational threat is the disjunction of hostile language and vocoded production. Disagreement floors and a calibrated OR recover complementary cells that a weighted sum misses, at a documented safe-cell false-alarm cost.
\item A sufficient-statistic agent, scored in a class-conditional simulator against an open-loop threshold, warns on human social engineering and does not spend its nonce there.
\item Residual features remain at chance on LPC after telephone-band filtering (AUC $0.49$). Pulse-formant is an easy unit condition, not a headline result. ASVspoof is not run.
\item Stage-aligned production change is posed and, on pulse-formant splices, \emph{not} supported. ACI does not beat a frozen quantile on the reported stream.
\end{itemize}
...
\begin{abstract}
Telephone fraud mixes social-engineering scripts with synthetic or vocoded speech, including mid-call production switches timed to a harvest ask. Utterance-level anti-spoofing and keyword products treat those as separate markets; weighted-sum fusion treats them as redundant. This paper formulates a streaming, telephony-conditioned, \emph{disjunctive} detection problem: a call is a threat if either stream is hostile. A channel twin (narrowband, $\mu$-law, packet loss, and neural-codec-like STFT quantization) is first-class evaluation law. Linguistic scoring is reported on a lexicon-locked independent set, not on author paraphrases written against the same emissions. A sufficient-statistic agent never sees waveforms or transcripts and spends at most one liveness challenge per call.

On a lexicon-locked independent English set, a wide frozen lexicon beats narrow keywords; an HMM stage tracker does not beat the wide bag, so discourse-HMM novelty is not claimed. Residual acoustics remain weak on LPC after bandlimiting. Disagreement floors change complementary-cell recall relative to a weighted sum, at a documented false-alarm cost; ranking AUC on constructed cells is not the headline. A sufficient-statistic agent is scored in a class-conditional simulator and, in a smaller closed loop, on pipeline scores. Stage-aligned production change is not supported on pulse-formant splices. Scope: offline evaluation of a research sidecar on English, single-talker, injected transcripts. It is not a state-of-the-art anti-spoofing encoder and not a U.S.\ deployment. Numbers in the PDF must match \texttt{docs/results/upgrade\_experiments.json} after the last confirmatory run.
\end{abstract}

\noindent\textit{Keywords:} vishing; voice spoofing; telephone channel; score fusion; change-point; conformal inference; active sensing; sufficient-statistic agent.
...
\begin{quote}
\textbf{RQ-B.} On independent transcripts, with the stage lexicon frozen first, does a trajectory model beat a \emph{wide} bag of the same emissions, and a narrow keyword list, with and without a 25\% ASR-noise model?

\textbf{RQ-C.} For the OR-label, do disagreement floors or a calibrated OR dominate a weighted sum on complementary cells at threshold $0.5$, and what is the safe-cell FPR?

\textbf{RQ-E.} Given only sufficient statistics, does a budgeted agent reduce wasted challenges on human social engineering relative to an open-loop threshold, in a class-conditional simulator?
\end{quote}

Author-written held-out paraphrases and pulse-formant EER remain as \emph{sanity} splits. They are not confirmatory. SAPC and residual-vs-LPC are reported as negatives in Results, not as extra research questions. We treat those failures as results, not as a prompt to retune until the tests pass.

\section{Related work}

\subsection{Utterance-level anti-spoofing and why telephony plus neural codecs break it}
ASVspoof established community protocols for logical access and replay~\cite{todisco2019asvspoof,wang2020asvspoof}. Competitive systems use self-supervised front ends and graph or raw-waveform backends~\cite{jung2022aasist,tak2021end}. ASVspoof~5 adds crowdsourced speech, neural codecs, PSTN simulation, and adversarial attacks~\cite{liu2023asvspoof5,wang2026asvspoof5}. Codec-based deepfakes (CodecFake and related ALM work) are a 2024--2026 TASLP concern~\cite{xie2025codecfake}. Those systems are the right comparison for neural TTS and voice conversion. We do not replace them and we do not report ASVspoof equal error rates: the inductive bias claimed here is telephony-first evaluation, not a leaderboard EER. Channel mismatch is a documented failure mode~\cite{zhang2021channel}. Our telephone-band simulator is a classical DSP chain plus Opus-like and neural-codec-like profiles. It is a stress test, not a learned generative model of the PSTN.

\subsection{Partial and segmental spoofing versus timing-to-discourse}
PartialSpoof constructed utterances in which only some segments are fake and showed that counters trained on fully spoofed data degrade~\cite{zhang2021partial,zhang2023partialspoof}. That line asks whether any frame is synthetic. It does not ask whether the fake onset is timed to a vishing stage, and it does not consume a linguistic stream. SAPC is that different question; it is currently unsupported on pulse-formant splices.

\begin{table}[t]
\centering
\caption{Positioning. Telephony-first means the channel is an evaluation law, not an afterthought. Privacy means the decision maker does not see transcript or waveform.}
\label{tab:pos}
\scriptsize
\begin{tabular}{lcccccc}
\toprule
System & Tel. & OR-label & Mid-call & Budget & Privacy & Open neg. \\
\midrule
ASVspoof CM (AASIST/RawNet2) & & & & & & \\
PartialSpoof & & & any-fake & & & \\
Late-average multimodal vishing & & & & & & \\
LLM call copilots & & & & & no & \\
ShieldCall (this work) & yes & yes & posed & yes & yes & yes \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Vishing and discourse}
Applied vishing detectors typically classify transcripts or average an audio score with a text score. Dialogue-act models assign speech to acts such as request and statement~\cite{stolcke2000dialogue}. Professional scam calls progress through greeting, claimed authority, invented problem, urgency, harvest, payment, secrecy, and threat. We test a locked-lexicon control: a \emph{wide bag} of the same stage emissions versus a narrow high-precision keyword list versus an HMM path prior versus a linear stage-count model. On an independent set the wide bag accounts for the lift; the HMM does not beat it. We do not claim to have invented dialogue-act tagging, and we do not claim the stage machine as a contribution.
...
Agent evaluation uses (i) a class-conditional score simulator against an open-loop threshold and (ii) a smaller closed loop in which the pipeline emits sufficient statistics from audio plus injected transcripts. Scripted percept traces remain an illustration, not the confirmatory table.

Sample sizes are small. We do not report $p$-values on $n{=}6$ as evidence of absence beyond the ranking AUC itself.

\section{Results}

\subsection{RQ-B: language, lexicon locked}
Confirmatory numbers use an independent set written after \texttt{STAGE\_EMISSIONS} was hashed and frozen ($n{=}40$). Narrow keywords (high-precision groups) reach AUC $0.547$ (bootstrap interval about $0.39$--$0.70$). The \emph{wide} bag of the same frozen emissions reaches AUC $0.839$ ($0.72$--$0.94$). The HMM stage tracker reaches $0.820$ ($0.69$--$0.93$) and does not beat the wide bag. A logistic trajectory model fit only on the author train split reaches $0.750$. Under a 25\% ASR-noise model the wide bag remains strongest ($0.84$). Isolated-keyword trap means are $0.50$ (narrow), $0.13$ (wide), $0.54$ (HMM). The path prior is therefore not the mechanism of the lift: the broader lexicon is. Author held-out paraphrases, on which keywords+stages had AUC $0.88$ versus $0.42$, are a sanity split and are contaminated by shared authorship with the lexicon.

\begin{table}[t]
\centering
\caption{Linguistic detection on the lexicon-locked independent set ($n{=}48$, corpus hash \texttt{7879918a4fe08c5d}). Trap is mean score on isolated-keyword benign lines (lower is better). Author held-out is sanity only. LTM is logistic regression on stage counts, not a neural net. Text-noise is not ASR WER. Source: \texttt{upgrade\_experiments.json}.}
\label{tab:ling}
\begin{tabular}{lcccc}
\toprule
Condition & $n$ & AUC & 95\% CI & Trap \\
\midrule
Independent, narrow keywords & 48 & 0.596 & 0.49--0.70 & 0.24 \\
Independent, wide lexicon & 48 & 0.836 & 0.73--0.92 & 0.10 \\
Independent, SDTG (HMM) & 48 & 0.817 & 0.71--0.92 & 0.33 \\
Independent, LTM (linear counts) & 48 & 0.733 & 0.59--0.86 & 0.10 \\
Independent, wide, text-noise $0.25$ & 48 & 0.785 & 0.65--0.89 & 0.07 \\
Author held-out, keywords (sanity) & 38 & 0.417 & --- & 0.23 \\
Author held-out, keywords+stages (sanity) & 38 & 0.883 & --- & 0.23 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Author-split sanity plot (not confirmatory). Confirmatory independent-set numbers are Table~\ref{tab:ling}.}
\label{fig:ling}
\end{figure}
...
\subsection{RQ-C: operational fusion}
...
\begin{table}[t]
\centering
\caption{Operational fusion on a constructed OR-label grid (sanity). Confirmatory fusion rows, including TPR at FPR $0.05$, are in \texttt{upgrade\_experiments.json}. Do not read AUC $1.000$ as a population result.}
\label{tab:fuse}
\begin{tabular}{lcccc}
\toprule
...
\subsection{RQ-E: agent}
A class-conditional simulator emitting sensor-like scores ($n{=}20$ traces per hypothesis) is the confirmatory policy table: the hand-set agent has missed-harvest $0$ and false-challenge $0$ on social engineering, i.e.\ it does not waste the nonce on a live scammer. An open-loop threshold also interrupts SE in this simulator; the distinction is the wasted-nonce rate, not recall. Table~\ref{tab:agent} still records the older scripted-percept illustration.
...
\begin{enumerate}
\item Independent scripts are still English, single-talker, and synthetic; they are not victim audio.
\item Audio is Mini LibriSpeech, not ASVspoof~5 and not CPaaS playback.
\item Pulse-formant is easy by construction and is not a headline result.
\item Fusion cells are constructed, not sampled live-like trajectories.
\item SAPC $n$ is a pilot; the gold $n{\ge}100$ streaming-VC construction is specified and not completed here.
\item Agent likelihoods are not learned; the simulator is class-conditional, not a live A/B.
\item No user study, no carrier deployment, no LLM transcript baseline in production (an LLM foil is specified as a baseline, not shipped).
\end{enumerate}

\subsection{What would change the claim}
ASVspoof~5 through TCT-2, with AASIST or RawNet2 in the same table, would change the acoustic negative. A streaming-VC gold SAPC construction would give the timing hypothesis a second chance to fail or succeed. Public scam transcripts with a native evaluator would test RQ-B outside the independent set written for this paper. Until then the claims remain as scoped above.

\section{Conclusion}

ShieldCall is a streaming dual-stream sensor plus a budgeted defense agent for telephone-bandwidth fraud. On a lexicon-locked independent set, a wide frozen lexicon beats narrow keywords and an HMM stage tracker does not beat the wide bag. Disagreement-aware fusion changes the recall and false-alarm tradeoff in the direction an operator would expect, at a price we report. Residual acoustics fail on LPC after bandlimiting. A timing statistic for mid-call handoffs works on clean point processes and does not on vocoded LibriSpeech splices. The agent warns on human vishing and spends its nonce where a vocoder might fail. Those sentences are the paper. Sentences about state-of-the-art deepfake detection, U.S.\ deployment, or a novel discourse HMM are not.
...
\section*{Ethics}
No real victim audio is used. Independent scripts are synthetic. A live-call pilot would require IRB review, recording consent, and TCPA analysis before collection. The sidecar is designed to store decisions, not waveforms. No LLM is on the decision path; a character-level ASR-noise model was used for the WER ablation. Seeds and the frozen config are in \texttt{configs/paper.yaml}.

\section*{Compute}
Linguistic and agent-simulator tables run on CPU in under a minute. Acoustic tables use Mini LibriSpeech on the evaluation host named in \texttt{docs/results/upgrade\_experiments.json}. No GPU is required for the reported numbers.

\section*{Data availability}
Mini LibriSpeech is obtained from OpenSLR~31. Independent script IDs, code, configs, and tabulated results are in the companion repository. ASVspoof is not redistributed; set \texttt{SHIELDCALL\_ASVSPOOF\_ROOT} to enable the loader. Reproduce with \texttt{pytest -q}, \texttt{python scripts/run\_upgrade\_experiments.py}, \texttt{python scripts/run\_paper\_experiments.py}, and \texttt{python scripts/run\_handoff\_experiment.py}. The protocol is public; regex weights may remain closed. We do not claim a released victim-call corpus.
