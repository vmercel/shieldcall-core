...
\begin{itemize}
\item On a lexicon-locked independent script set ($n{=}40$), a wide bag of the frozen stage lexicon reaches AUC $0.84$ versus $0.55$ for narrow keywords; the HMM stage tracker does \emph{not} beat the wide bag (AUC $0.82$).
...
On forty independent English scripts, a wide frozen lexicon reaches AUC $0.84$ (bootstrap interval about $0.72$--$0.94$) against $0.55$ for narrow keywords; an HMM stage tracker does not beat the wide bag. Residual acoustics remain at chance on LPC after bandlimiting (AUC $0.49$). Disagreement floors recover complementary-cell recall that a naive sum misses, at a safe-cell false-positive cost we report. A class-conditional agent simulator warns on human social engineering and does not waste the nonce there. Stage-aligned production change is not supported on pulse-formant splices. Scope: offline evaluation of a research sidecar on English, single-talker, injected transcripts. It is not a state-of-the-art anti-spoofing encoder and not a U.S.\ deployment.
\end{abstract}
...
\begin{table}[t]
\centering
\caption{Linguistic detection on the lexicon-locked independent set ($n{=}40$). Trap is mean score on isolated-keyword benign lines (lower is better). Author held-out is sanity only.}
\label{tab:ling}
\begin{tabular}{lcccc}
\toprule
Condition & $n$ & AUC & 95\% CI & Trap \\
\midrule
Independent, narrow keywords & 40 & 0.547 & 0.39--0.70 & 0.50 \\
Independent, wide lexicon & 40 & 0.839 & 0.72--0.94 & 0.13 \\
Independent, SDTG (HMM) & 40 & 0.820 & 0.69--0.93 & 0.54 \\
Independent, NTM & 40 & 0.750 & 0.59--0.88 & 0.19 \\
Independent, wide, ASR 25\% WER & 40 & 0.840 & --- & 0.03 \\
