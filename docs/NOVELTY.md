...
Locked lexicon: stage hash `LEXICON_LOCK` plus combined `current_lexicon_lock()` in `shieldcall/linguistic/lock.py` (emissions + `PATTERN_GROUPS`).
Frozen hyperparameters: `configs/paper.yaml`.
Contamination log: `docs/lab/CONTAMINATION.md`.
Independent corpus hash: `independent_corpus_hash()` in `eval/corpora/independent_scripts.py`.
...
| Pulse-formant after bandlimit | EER 0, *n*=20, easy | Demoted to unit test | Headline vocoder: **LPC only**. `neural_quant` is sanity. |
| Residual vs LPC+NB | AUC 0.49 (*n*=20 historical) | Hybrid-H vs published neural baseline on TCT-2 | Hybrid-H = logistic on residual embeddings. AASIST still missing. |
...
| Agent warn-on-SE | 3 scripted traces | Simulator + closed-loop sensor scores vs threshold | Simulator + `agent_closed_loop` in upgrade JSON |
...
6. **Closed-loop (pipeline scores):** on bona fide LibriSpeech + independent scam text, the agent still interrupts SE but **spent the nonce** (SE false_challenge $1.0$, $n{=}4$). The warn-not-challenge policy does **not** yet survive real sensor scores. Keep as a negative/open item.
7. **Hybrid-H** logistic on residual embeddings: LPC narrowband AUC $0.91$ vs PMA $0.59$ at $n{=}32$ (64-D, small $n$). Treat as exploratory until regularised / nested CV / AASIST table exists.
...
| HiFi-GAN / Encodec detection | `neural_quant` / TCT `NEURAL_CODEC` are STFT caricatures |
| Production ASR | `synthetic_text_noise`; Whisper unused |
